"""South African PII detection and masking guard."""
...
class _PiiZa:
    ...
    def check(
        ...
        return GuardResult(
            ...
            passed=False,
            action=self.action,
            message="South African PII detected in text",
            latency_ms=round(elapsed, 2),
            details={
                "detected": detected,
                "reason": (
                    "Text contains South African personally"
...
def pii_za(
    *,
    ...
    return _PiiZa(action=action, entities=entities)
