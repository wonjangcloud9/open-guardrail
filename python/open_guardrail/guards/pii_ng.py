"""Nigerian PII detection and masking guard."""
...
class _PiiNg:
    ...
    def check(
        ...
        return GuardResult(
            ...
            passed=False,
            action=self.action,
            message="Nigerian PII detected in text",
            latency_ms=round(elapsed, 2),
            details={
                "detected": detected,
                "reason": (
                    "Text contains Nigerian personally"
...
def pii_ng(
    *,
    ...
    return _PiiNg(action=action, entities=entities)
